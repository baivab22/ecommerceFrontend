  ...
  if (looksAggregated && !isOrderItem(rawTopProducts[0])) {
    products = (rawTopProducts as AggregatedTopProductItem[])
      .map((item, index) => {
        ...
        // SAFE ACCESS: Check if productDetails exists and has a valid name
        let productName = 'Unknown Product';
        if (item.productDetails && Array.isArray(item.productDetails) && item.productDetails.length > 0) {
          productName = item.productDetails[0]?.name || 'Unknown Product';
        }
    ...
    (rawTopProducts as OrderItem[]).forEach((order) => {
      ...
      order.products.forEach((item, itemIndex) => {
        // SAFE ACCESS: Handle null/undefined productId
        if (!item || !item.productId) {
          const fallbackId = `unknown-${order._id || 'order'}-${itemIndex}`;
          if (!productMap[fallbackId]) {
            productMap[fallbackId] = {
          ...
          const quantity = toNumber(item.quantity);
          const lineRevenue = quantity * toNumber(item.price);
          productMap[fallbackId].totalQuantity += quantity;
          productMap[fallbackId].totalRevenue += lineRevenue;
          return;
        }

        // SAFE ACCESS: Handle different productId types
        ...
        if (typeof item.productId === 'string') {
          ...
          productId = productObj?._id || productObj?.id || `unknown-${order._id}-${itemIndex}`;
          // CRITICAL FIX: Check if productObj exists and has name property
  ...
  const orderBreakdown = useMemo<OrderBreakdownData[]>(() => {
    return filteredOrders
      .map((order) => {
        ...
        // FIX: Safely access product name to prevent null errors
        const productSummary = order.products
          .slice(0, 3)
          .map((item) => {
            ...
            const quantity = toNumber(item.quantity);
            const unitPrice = quantity > 0 ? lineTotal / quantity : lineTotal;
            return `${productName} x${quantity} @ ${formatCurrency(unitPrice)} = ${formatCurrency(lineTotal)}`;
          })
          .join(', ');
        ...
        return {
          ...
          computedTotal,
          totalAmount: actualTotal,
          productSummary: productSummary || 'No product details available',
        };
      })
      .sort((a, b) => new Date(b.scannedAt).getTime() - new Date(a.scannedAt).getTime());
  }, [filteredOrders]);
