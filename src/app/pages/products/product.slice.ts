...
const updateProductAction = createAsyncThunk(
  'product/update', // Fixed typo: 'udpate' -> 'update'
  async (
    {
      productBody,
      productId,
...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    // Optional: Add a complete reset function if needed
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getProductDetailByIdAction.pending, (state) => {
      state.productDetailLoading = true  // Fixed: was setting 'loading' instead of 'productDetailLoading'
    })
    builder.addCase(getProductDetailByIdAction.fulfilled, (state, action) => {
      state.productDetailLoading = false
      state.productDetailData = action.payload.data
...
// Export all actions
export const { resetProductDetail, resetProductState } = productSlice.actions

// Export all thunk actions
export {
  getProductListAction,
  getHotSellingProductsAction,
  getWatchAndShopProductsAction,
