...
//   // Fetch current hot selling products
//   const fetchHotSellingProducts = useCallback(async () => {
//     try {
//       const response = await axios.get(`${BASE_URL}/products/hot-selling`);
//       setHotSellingProducts(response.data.data || []);
//     } catch (error) {
...
//       showMessage('success', response.data.message || 'Product added to hot selling!');
//       await Promise.all([fetchHotSellingProducts(), fetchProducts(currentPage, searchTerm)]);
//     } catch (error) {
//       console.error('Error adding to hot selling:', error);
//       showMessage('error', error.response?.data?.error || 'Failed to add product');
//     } finally {
...
//     fetchProducts();
//     fetchHotSellingProducts();
//   }, [fetchProducts, fetchHotSellingProducts]);
...
//   const isHotSelling = useCallback((productId) => {
//     return hotSellingProducts.some(p => p._id === productId || p.id === productId);
//   }, [hotSellingProducts]);
...
//   const hotSellingIds = useMemo(() => {
//     return new Set(hotSellingProducts.map(p => p._id || p.id));
//   }, [hotSellingProducts]);
...
//             <p>Curate and manage your featured products collection</p>
//           </div>
//         </div>
//         <div className="header-stats">
//           <div className="stat-card">
//             <span className="stat-label">Featured Products</span>
//             <span className="stat-value">{hotSellingProducts.length}</span>
//           </div>
//           <div className="stat-card">
...
//           </div>
//           <span className="featured-badge">{hotSellingProducts.length} Products</span>
//         </div>
...
//             <p>Browse the product catalog below and click "Add to Featured" to showcase your best sellers</p>
//           </div>
//         ) : (
//           <div className="featured-grid">
//             {hotSellingProducts.map(product => (
//               <div key={product._id || product.id} className="featured-card">
//                 <div className="card-media">
//                   <img 
//                     src={getProductImage(product)} 
//                     alt={product.name}
//                     onError={(e) => e.target.src = '/assets/images/defaultProduct.jpeg'}
//                   />
//                   {getDiscountPercentage(product) > 0 && (
//                     <div className="discount-tag">-{getDiscountPercentage(product)}%</div>
//                   )}
//                   <div className="featured-tag">HOT</div>
//                 </div>
...
//                     <span className="current-price">{formatPrice(product.discountedPrice || product.originalPrice)}</span>
//                     {product.originalPrice > product.discountedPrice && (
...
//                     onClick={() => removeFromHotSelling(product._id || product.id)}
//                     disabled={actionLoading === (product._id || product.id)}
//                   >
//                     {actionLoading === (product._id || product.id) ? (
...
//                 products.map(product => {
//                   const productId = product._id || product.id;
//                   const isHot = hotSellingIds.has(productId);
                  
//                   return (
//                     <div key={productId} className={`product-card ${isHot ? 'featured' : ''}`}>
//                       <div className="card-media">
//                         <img 
//                           src={getProductImage(product)} 
//                           alt={product.name}
...
//                           <div className="discount-tag">-{getDiscountPercentage(product)}%</div>
//                         )}
//                         {isHot && <div className="featured-tag featured">HOT</div>}
//                       </div>
//                       <div className="card-content">
//                         <h3 className="product-title">{product.name}</h3>
//                         {product.category?.name && (
...
//                         )}
//                         <button 
//                           className={`btn-action ${isHot ? 'remove' : 'add'}`}
//                           onClick={() => isHot ? removeFromHotSelling(productId) : addToHotSelling(productId)}
...
//   // Set product as featured (automatically removes previous featured product)
...
//     return featuredProduct && (featuredProduct._id === productId || featuredProduct.id === productId);
//   }, [featuredProduct]);
...
//             <p>Browse the product catalog below and click "Set as Featured" to showcase your best product</p>
//           </div>
//         ) : (
